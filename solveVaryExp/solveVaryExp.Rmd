...
\begin{align}
\psi^{(c)}(j\tau | x) := d(x) \psi^{{(c)}}_{\rm loc}(j\tau | x) 
+ (1-d(x)) \psi^{(c)}_{\rm nonloc}(j\tau|x), \quad j=1,2,\ldots
\label{eq:def-psi}
\end{align}
The local part is simply deterministic, with all mass at $\tau$, that is 
$\psi^{(c)}_{\rm loc}(\tau | x) = 1$. 
...
```{r varyexp1, include=TRUE, echo=FALSE, message=FALSE, fig.cap="A medium which is more 'anomalous' to the left than to the right, see anomalous exponent (dashed line)."}
...
out <- DTSM(nuTail = nuTail, snapshots = 2^(1:5), c = 36, d = function(x) 0) %>% 
  rho_df() 
...
```{r varyexp2, include=TRUE, message=FALSE, echo=FALSE, fig.cap="An interface problem."}
#Parameters
alpha <- function(x)
...
out <- DTSM(nuTail = nuTail, snapshots = 2^(-1:4), c = 225, d = function(x) 0) %>% rho_df()
...
```{r varymixture, include=TRUE, echo=FALSE, message=FALSE}
#Parameters
alpha <- function(x)
  0.55 * exp(-x^2) + 0.15 + 0.5 / (1 + exp(-2 * x))
Psi <- function(x,t) {
...
```{r varytempering, include=TRUE, echo=FALSE, message=FALSE}
#Parameters
alpha <- function(x)
  0.5 / (1 + exp(-x)) + 0.25
theta <- function(x)
