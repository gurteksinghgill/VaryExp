...
abstract: |
  Continuous Time Random Walks (CTRWs) are widely used models of subdiffusive
  stochastic transport. The literature contains a great number of specialized
  algorithms for the computation of CTRW probability densities, which makes
  it difficult for an applied scientist to get started. This article addresses this
  problem by introducing a universal computational framework which applies to
  any CTRW (limit) process.
  
  Our algorithm leverages a representation of CTRWs via bivariate Levy pro-
  cesses. These Levy processes are discretized, to produce generalized master
  
  equations. It resembles the recent Discrete Time Random Walk (DTRW) algo-
  rithm, and shares the same desirable features: consistency, conservation of mass,
  
  and strictly non-negative solutions. Moreover, it provides a novel approach to
  solving fractional Fokker-Planck equations of variable order, which model media
  with spatially varying disorder. Using our framework, we also uncover a novel
  model for the cross-over from diffusive behaviour at early times to subdiffusive
  behaviour at late times, which we term “early tempering”.
  We highlight the applicability of our algorithm by computing i) solutions to
  fractional Fokker-Planck equations of variable order, ii) probability densities of
  early tempered subdiffusion, and iii) probability densities of generalized inverse
  subordinators.
