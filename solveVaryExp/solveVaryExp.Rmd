...
which prevents the Levy measure from blowing up as $\beta(y) \uparrow 1$, 
see Lemma \ref{lem:psi}.
...
\end{align*}
where $\mathcal O(c\tau)$ is the error in the approximation of the Riemann 
sum by the Riemann integral, 
and the final error term in $\varepsilon$ is due to \eqref{eq:blowup}. 
\qed
