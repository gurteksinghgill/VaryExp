...
abstract: |
  ...
  continuum limits.
  The algorithm is "universal", in the sense that it applies to any CTRW with 
  Any CTRW continuum limit
...
```{r interface, include=TRUE, message=FALSE, echo=FALSE, fig.cap="\\label{fig:interface}Continuous interface problem. Coefficients are as given in the text, and $c = 400$."}
...
```{r varying-temporal-drift, message=FALSE, echo=FALSE, fig.cap="\\label{fig:varying-temporal-drift}A system with a spatially varying temporal drift $d(x) = 10 * \\sin^2(2\\pi x)$. Particles accumulate in the slow patches where $d(x)$ is high, while trapping is homogeneous in space. Other parameters: $a(x,t) = 0.9$, $b(x,t) = 0$, $\\overline\\nu(w|x) = w^{-0.7}/\\Gamma(1-0.7)$, $c = 900$."}
d <- function(x,t){
  10 * (sin(2*pi*x))^2
}
