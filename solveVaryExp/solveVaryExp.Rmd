...
This remains true even if the speed varies with the location $(x,t)$ of 
$(Y_u,Z_u)$.
...
as follows: Define $\theta(x)$ via $d(x) = \theta(x) / (1-\theta(x))$. 
...
\label{eq:def-psi}
\end{align}
where, by definition, $0 \le d(x) \le 1$.
...
\\ \label{eq:cond3}
\lim_{\epsilon \downarrow 0} \lim_{c \to \infty}
c \iint\limits_{|z|< \epsilon, \,0<w < \epsilon} w K^{(c)}(z,w | x,s)\,dz\,dw &= \theta(x)
\\
\label{eq:cond4}
\lim_{c \to \infty}
c \iint\limits_{z \in \mathbb R, w \ge 0} g(z,w) K^{(c)} (z,w | x,s)\,dz\,dw &= \int_{w > 0} g(0,w) (1-\theta(x)) \nu(w|x)\,dw
\end{align}
...
```{r interface, include=TRUE, message=FALSE, echo=FALSE, fig.cap="\\label{fig:interface}Continuous interface problem. Coefficients are as given in the text. Other numeric parameters are $c = 100$, $\\chi = 1/10$ and $\\tau = 1/100$."}
...
either side of the interface. For late times we can see the aggregation of all 
particles towards the left hand side ($y<0$) where trapping is stronger.


@Straka17 shows that changing time units from $T_0 = 1$ to $T_0 = 2$ results 
in the the updated diffusivity and drift coefficients 
\begin{align*}
a_{\beta(y)}(x,t) = \frac{a(x,t)}{T_0^{-\alpha(x)}}, 
\quad 
b_{\beta(y)}(x,t) = \frac{b(x,t)}{T_0^{-\alpha(x)}},
\end{align*}
...
$(\overline \nu(w|x), d(x), a_{\alpha(x)}(x,t), b_{\alpha(x)}(x))$, at the 
timestamps multiplied by $T_0 = 2$, and plotting the differences
(Figure \ref{fig:difference}). 
Relative differences are near 1\%, and remain stable after 8 units of time,
indicating that indeed the same densities are calculated in both cases. 


```{r difference, eval=TRUE, message=FALSE, echo=FALSE, fig.cap="\\label{fig:difference}Absolute difference between densities, calculated at corresponding timestamps, for two choices of time scale $T_0 = 1$ and $T_0 = 2$."}
T0 <- 2
...
Gaussian process. Figure \ref{fig:varying-temporal-drift} shows anomalous diffusion 
...
```{r temporal-drift, echo=FALSE, message=FALSE, fig.cap="\\label{fig:temporal-drift}Increasing temporal drift $d(x)$ decreases the speed of the diffusion and increases resemblence to a standard Gaussian process. Parameters: $a(x,t) = 0.8, b(x,t) = 0, \\overline\\nu(w|x)=w^{-0.7}/\\Gamma(1-0.7), c=100, \\tau = 1/100, \\chi = 1/10$."}
...
```{r varying-temporal-drift, message=FALSE, echo=FALSE, fig.cap="\\label{fig:varying-temporal-drift}A system with a spatially varying temporal drift $d(x) = 10 * \\sin(2\\pi x)$. Particles accumulate in the slow patches where $d(x)$ is high, while trapping is homogeneous in space. Other parameters: $a(x,t) = 0.8, b(x,t) = 0, \\overline\\nu(w|x) = w^{-0.7}/\\Gamma(1-0.7), c = 900, \\tau = 1/900, \\chi = 1/30$."}
d <- function(x,t){
  10 * (sin(2*pi*x))^2
}
...
Anomalous diffusion with _distributed order_ assumes a mixing probability distribution 
$p(\beta)\,d\beta$ on the interval $(0,1]$, and the 
...
```{r varymixture, include=TRUE, echo=FALSE, message=FALSE, fig.cap="\\label{fig:varying-mixture}Distributed order anomalous diffusion with spatially varying mixture. The mixture has two modes, $\\beta_1 = 0.7$ (left) and $\\beta_2 = 0.3$ (right). The mixing is according to $(p(x), 1-p(x))$, where $p(x) = 1 / (1 + \\exp(-2x)).$"}
p <- function(x)
  1 / (1 + exp(-2 * x))
nuTail1 <- function(x,t) {
  if (t <= 0)
    Inf
  else
    (t^(-0.3)  )/gamma(1-0.3)
}
nuTail2 <- function(x,t) {
  if (t <= 0)
    Inf
  else
    (t^(-0.7)  )/gamma(1-0.7)
}
mixedTail <- function(x, t)
  p(x) * nuTail1(x, t) + (1 - p(x)) * nuTail2(x, t)

DTSM(nuTail = mixedTail, snapshots = 2^(1:5)) %>%
  rho_df() %>%
...
## Inverse tempered stable subordinator

```{r, message=FALSE, echo=FALSE, fig.cap="Probability densities of the tempered inverse stable subordinator."}
nuTail <- function(x,w){
  beta <- 0.7
  gam <- 1
  (w ^ (-beta) * exp(-gam * w) - gam * pracma::gammainc(w, 1 - beta)) / gamma(1-beta)
}
DTSM(
  c = 20 ^ 2,
  nuTail = function(x,t){
