---
title: |
  A Universal Algorithm for Continuous Time Random Walk Limit Distributions
author:
  - name: Gurtek Gill
...
abstract: |
  ...
  limits, as we illustrate with four worked examples. 
  ...
  of most CTRW limit distributions.
journal: "a journal"
date: "`r Sys.Date()`"
...
\label{eq:FFPE}
\end{align}
The memory kernel corresponds uniquely to the Levy coefficient functions 
$d(y)$ and $\bar \nu(w|y)$ from Equations \eqref{eq:dZ} and \eqref{eq:nubar}
via 
$$
\hat V(y, \lambda) = \frac{1}{\lambda [d(y) + \hat{\bar \nu}(\lambda |y)]}
$$
where a hat denotes the Laplace transform $s \rightsquigarrow \lambda$
in the $s$ variable only [@BaeumerStraka16; @Straka17].
In the following table, we collect a few specifications of $\hat V(y,\lambda)$
and $\hat {\bar \nu}(\lambda | y)$ from the literature:

|                       | $\hat V(y,\lambda)$        | $\bar\nu(w\vert y)$ | Reference |
| ----------------:     | :-------------: |:-----------------:| :--------------: |
| memoryless            | $\lambda^{-1}$ | 0 |
| subdiffusion          | $\lambda^{-\alpha}$ | $w^{-\alpha}/\Gamma(1-\alpha)$ | @Sokolov2006e |
| tempered subdiffusion | $[(\lambda + \eta)^\alpha - \eta^\alpha]^{-1}$ | $\frac{w^{-\alpha} e^{-\eta w} - \eta \Gamma(1-\alpha, w)}{\Gamma(1-\alpha)}$ | @Gajda2010 |
| distributed order (Caputo form) | $\left[\int_0^1 (\tau \lambda)^\alpha p(\alpha)\,d\alpha\right]^{-1}$ | $\int_0^1 \frac{\tau^\alpha w^{-\alpha}}{\Gamma(1-\alpha)} p(\alpha)\,d\alpha$ | @Meerschaert2006ultra[^1]
| distributed order (RL-form) | $\int_0^1(\tau \lambda)^{-\alpha} p(\alpha)\,d\alpha$ | no closed form[^2] | @Mainardi08
| variable order | $\lambda^{-\alpha(y)}$ | $w^{-\alpha(y)}/\Gamma(1-\alpha(y))$ | @Chechkin2005a |
| spatially varying tempered subdiffusion   | $[(\lambda + \eta(y))^{\alpha(y)} - \eta^{\alpha(y)}]^{-1}$ | $\frac{w^{-\alpha(y)} e^{-\eta(y) w} - \eta(y) \Gamma(1-\alpha(y), w)}{\Gamma(1-\alpha(y))}$ | @Straka17

[^1]: We have added the factor $\tau^\alpha$, where $\tau$ is a time scale, 
    in order to make dimensions explicit. The factor $\Gamma(1-\alpha)^{-1}$
    was added to unify the presentation.
[^2]: The tail function $\bar \nu(w | y)$ can be obtained by finding 
    the inverse Laplace transform 
    $\mathcal L^{-1}\left\lbrace\left[\lambda \int_0^1(\tau \lambda)^{-\alpha} p(\alpha)\,d\alpha\right]^{-1}\right\rbrace$
[^3]: For a discussion of the few works that did study variable order problems, 
    see @Straka17. 


Up to this article (see Section 5), anomalous diffusion problems with 
positive temporal drift $d(y) > 0$ have not been considered, to the best of our
knowledge, and hence throughout the table $d(y) \equiv 0$. 
Moreover, the vast majority of works in numerical mathematics have studied
spatially constant memory kernels, without any dependence on the space variable
[^3] $y$. 
This implies a homogeneous distribution of waiting times throughout the
entire medium, i.e. that diffusion is equally
anomalous everywhere.  This assumption is too restrictive for some
...
Hence it applies to all cases listed in the above table, except possibly 
the distributed order (RL-form) case, for which $\bar \nu(w|y)$ is not readily 
available.
...
on $y$ [@MMMPS12]; 
that is, assuming that $y$ was frozen, 
$V(y,s)$ is the mean amount of auxiliary time $u$ for which $Z_u < s$.
...
Then, define $w_\tau$ as the largest grid point not larger than $w$:
\begin{align}
...
In this section, we apply our algorithm to four novel examples. 
All source code is openly available at \url{https://github.com/strakaps/VaryExp} [@varyExp]. 
Computations and plots were produced using the `R` programming
language [@R] using the packages `rmarkdown` [@rmarkdown] 
and `rticles` [@rticles].
