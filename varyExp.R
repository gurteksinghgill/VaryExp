...
xmax <- round(10/chi)*chi                                     #range of x = (-xmax, xmax)
t <- round(100/tau)*tau                                        #time at which evaluate density
...
Sprob <- cbind(TailF[,1], TailF[, seq(2,ncol(TailF),1)]/TailF[, seq(1,ncol(TailF)-1,1)] )
...
Jprob <- rbind(c(0.5, 1 - a(seq(-xmax+chi,xmax-chi,chi),i*tau)                                           ,0.5),
               c(0.5, (a(seq(-xmax+chi,xmax-chi,chi),i*tau) + chi*b(seq(-xmax+chi,xmax-chi,chi),i*tau))/2 , 0), 
               c(0, (a(seq(-xmax+chi,xmax-chi,chi),i*tau) - chi*b(seq(-xmax+chi,xmax-chi,chi),i*tau))/2 , 0.5) )
...
par(mfrow=c(2,2))
plot(seq(-xmax,xmax,chi),rowSums(xi0),type="l", main = "Rho distribution at t = 0", xlab = "x", ylab = "Rho (x,0)")
grid()
plot(seq(0,vmax,tau),colSums(xi0),type="l", main = "Residences times distribution at t = 0", xlab = "v", ylab = "Sum_{x} Xi (x,v,0)")
grid()
plot(seq(-xmax,xmax,chi),rowSums(xi)/chi,type="l", main = paste("Rho distribution at t = ", t), xlab = "x", ylab = paste("Rho (x,", t, ")"))
grid()
